...
function openFont($font){
  ...
  if ($pos===false){
    //$dir = './';
	$dir = CORE_APP.'vendors/ezpdf/fonts/';
    $name = $font;
  } else {
...
function selectFont($fontName,$encoding='',$set=1){
  $fontName = CORE_PATH . 'vendors/ezpdf/fonts/'.$fontName;
...
function setCurrentFont(){
  ...
  $cf = substr($this->currentBaseFont,strrpos($this->currentBaseFont,'/ezpdf/fonts/')+1);
  if (strlen($this->currentTextState)
    ...
    $nf = substr($this->currentBaseFont,0,strrpos($this->currentBaseFont,'/ezpdf/fonts/')+1).$this->fontFamilies[$cf][$this->currentTextState];
...
function addText($x,$y,$size,$text,$angle=0,$wordSpaceAdjust=0){
  if (!$this->numFonts){$this->selectFont('Helvetica.afm');}
