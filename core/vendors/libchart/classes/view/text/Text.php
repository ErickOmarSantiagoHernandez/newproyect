    ...
    class Text {
        ...
        public function printText($img, $px, $py, $color, $text, $fontFileName, $align = 0) {
            ...
            $lineSpacing = 1;
             ...
             list ($llx, $lly, $lrx, $lry, $urx, $ury, $ulx, $uly) = imageftbbox($this->fontSize, 0, $fontFileName, $text, array("linespacing" => $lineSpacing));
            ...
            imagettftext($img, $this->fontSize, $angle, $px, $py, $color->getColor($img), $fontFileName, $text);
        }
        ...
        public function printDiagonal($img, $px, $py, $color, $text) {
            //$this->fontSizeDiagonal = 7;
            $fontFileName = $this->fontCondensed;

            $lineSpacing = 1;

             list ($lx, $ly, $rx, $ry) = imageftbbox($this->fontSizeDiagonal, 0, $fontFileName, $text, array("linespacing" => $lineSpacing));
            $textWidth = $rx - $lx;

            //$this->angle = -40;

            imagettftext($img, $this->fontSizeDiagonal, $this->angle, $px, $py, $color->getColor($img), $fontFileName, $text);
        }
		
		public function setFontSize($fontSize=8){
			$this->fontSize = $fontSize;
		}
		
		public function setDiagonal($fontSize=8,$angle=-40){
			$this->fontSizeDiagonal = $fontSize;
			$this->angle = $angle;
		}
    }
?>