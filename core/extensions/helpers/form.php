...
class Form
{
    ...
    public static function getFieldData($field, $value = null, $filter = true)
    {
        ...
        if ($value !== null && $filter) {
            $value = htmlspecialchars(utf8_encode($value), ENT_COMPAT, APP_CHARSET);
        }
     ...
     * @return mixed
     */	
    public static function getFieldDataExt($field, $value = null, $filter = true)
    {
        // Obtiene considerando el patrón de formato form.field
        $formField = explode('.', $field, 2);
    ...
    public static function getFieldValue($field, $filter = true)
    {
        ...
        // Filtrar caracteres especiales
        if ($value !== null && $filter) {
            return htmlspecialchars(utf8_encode($value), ENT_COMPAT, APP_CHARSET);
        }
	...
	public static function fecha($field, $class = NULL, $attrs = NULL, $value = NULL)
    {
        if (is_array($attrs)) {
            $attrs = Tag::getAttrs($attrs);
        }
    ...
    public static function select($field, $data, $attrs = NULL, $value = NULL)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            $options .= '>' . htmlspecialchars(utf8_encode($v), ENT_COMPAT, APP_CHARSET) . '</option>';
        }

        return "<select id=\"$id\" name=\"$name\" $attrs>$options</select>";
    }
	
    public static function lista($field, $data, $attrs = NULL, $value = NULL)
    {
        if (is_array($attrs)) {
            $attrs = Tag::getAttrs($attrs);
        }
        ...
        // Obtiene name, id y value (solo para autoload) para el campo y los carga en el scope
        extract(self::getFieldDataExt($field, $value), EXTR_OVERWRITE);
    ...
    public static function dbSelect($field, $show = NULL, $data = NULL, $blank = 'Seleccione', $attrs = NULL, $value = NULL)
    {
        ...
        foreach ($data as $p) {
            ...
            $options .= '>' . htmlspecialchars(utf8_encode($p->$show), ENT_COMPAT, APP_CHARSET) . '</option>';
        }
    ...
    public static function textarea($field, $attrs = NULL, $value = NULL)
    {
        ...
        //return "<textarea id=\"$id\" name=\"$name\" $attrs>".utf8_encode($value)."</textarea>";
